...
public class AppPeliculasEjer3 {
	public static void main(String[] args)  {
		...
		Pantalla.escribirString("\nPeliculas de mÃ¡s de 6 meses de creaciÃ³n");
		for (Pelicula p: peliculas)
			if (ChronoUnit.MONTHS.between(p.getFechaCreacion(), p.getFechaEstreno())> 6)
				Pantalla.escribirString("\n" + p);
		
		// Mostrar la primera una pelÃ­cula con precio superir a 50 euros
		...
		if (encontrado)
			Pantalla.escribirString("\nLa peli que vale mÃ¡s de 50 es :" + pelicula);
		else
			Pantalla.escribirString("\nNo hay peli que vale mÃ¡s de 50 " );
